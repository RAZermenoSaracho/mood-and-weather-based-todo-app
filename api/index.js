...
// ---------------------------------------------------------
// ROOT DIR (NO redeclarar __dirname)
// ---------------------------------------------------------
...
// Importante: NO bloquear el arranque, pero sí conectar
connectDB();
...
app.use(
    session({
        ...
        cookie: {
            httpOnly: true,
            secure: isVercel || isProd, // local=false, vercel/prod=true
...
app.get("/", async (req, res) => {
    try {
        if (!req.session.userId) {
            return res.render("pages/index", { tasks: [] });
        }
...
// ---------------------------------------------------------
// WEATHER API PROXY  ✅ (esto lo requiere weather.js)
// ---------------------------------------------------------
...
app.patch("/auth/me", async (req, res) => {
    try {
        ...
        if (location === null) {
            ...
            updates.location = trimmed === "" || trimmed.toLowerCase() === "none" ? null : trimmed;
        }
        ...
        if (typeof email === "string" && email.trim()) {
            ...
            if (emailUsed) return res.status(400).json({ error: "Email already in use" });
        ...
        const user = await User.findByIdAndUpdate(req.session.userId, updates, { new: true }).select(
            "-password"
        );
...
// ---------------------------------------------------------
// TASKS CRUD (USER SCOPED) ✅ (esto lo requiere suggestedTaskComponent.js)
// ---------------------------------------------------------
...
app.patch("/tasks/complete-all", requireAuth, async (req, res) => {
    try {
        ...
        return res.status(500).json({ error: "Failed to complete all tasks" });
    }
});
...
app.post("/tasks", requireAuth, async (req, res) => {
    try {
        ...
        if (!name || !date || !weather) {
            return res.status(400).json({ error: "Missing required fields (name, date, weather)" });
        }
